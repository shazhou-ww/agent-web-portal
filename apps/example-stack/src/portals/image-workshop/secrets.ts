...
// Secret paths in AWS Secrets Manager (using awp-examples prefix)
const SECRET_PATHS = {
  STABILITY_API_KEY: "awp-examples/stability-api-key",
  BFL_API_KEY: "awp-examples/bfl-api-key",
} as const;
...
export async function getStabilityApiKey(): Promise<string> {
  ...
  const apiKey = await getSecretValue(SECRET_PATHS.STABILITY_API_KEY);
...
export async function getBflApiKey(): Promise<string> {
  ...
  // Fetch from Secrets Manager
  const apiKey = await getSecretValue(SECRET_PATHS.BFL_API_KEY);
  setCache(cacheKey, apiKey);
  return apiKey;
}
