 ...
 * BFL API base URL.
 *
 * As of 2026, the primary global endpoint is `https://api.bfl.ai`.
 * We keep this configurable for environments with allow-lists / regional routing.
 */
...
export interface BflSubmitResponse {
  id: string;
  /**
   * When using `api.bfl.ai` (global) or regional endpoints, the API returns a
   * `polling_url` that MUST be used for polling.
   */
  pollingUrl?: string;
}
...
function resolvePollingUrlOrLegacyTaskId(pollingUrlOrTaskId: string): string {
  // New-style: absolute polling URL returned by the API (preferred).
  if (pollingUrlOrTaskId.startsWith("http://") || pollingUrlOrTaskId.startsWith("https://")) {
    return pollingUrlOrTaskId;
  }

  // Some environments may return relative polling URLs.
  if (pollingUrlOrTaskId.startsWith("/")) {
    return `${getBflApiHost()}${pollingUrlOrTaskId}`;
  }

  // Legacy-style: we only have an id, so build the old get_result URL.
