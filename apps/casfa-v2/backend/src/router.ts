...
export const createRouter = (deps: RouterDeps): Hono<Env> => {
  ...
  app.post(
    "/api/auth/clients/init",
    zValidator("json", AwpAuthInitSchema),
    deps.authClients.init
  )
  ...
  app.get("/api/admin/users", deps.authMiddleware, deps.adminAccessMiddleware, deps.admin.listUsers)
  app.post(
