...
const mcpError = (id: string | number, code: number, message: string, data?: unknown): McpResponse => ({
  jsonrpc: "2.0",
  id,
...
export const createMcpController = (deps: McpHandlerDeps): McpController => {
  ...
  const handleWrite = async (id: string | number, args: unknown): Promise<McpResponse> => {
    ...
    return mcpError(id, MCP_INTERNAL_ERROR, "Write operation requires full CAS node creation - use HTTP API")
  }
