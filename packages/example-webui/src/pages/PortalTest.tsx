...
export default function PortalTest() {
  ...
  // Schema view toggle (Tool-facing vs LLM-facing)
  const [showLlmFacing, setShowLlmFacing] = useState(false);
  ...
  const updateDefaultArgs = (tool: Tool) => {
    ...
    if (schema && typeof schema === 'object' && 'properties' in schema) {
      ...
      for (const [key, prop] of Object.entries(props)) {
        // Skip output blob fields - they are auto-generated by the client
        if (outputBlobFields.includes(key)) {
          continue;
        }

        // For input blobs, use LLM-facing format: { uri: "" }
        if (inputBlobFields.includes(key)) {
          defaultArgs[key] = { uri: '' };
          continue;
        }
  ...
  /**
   * Convert Tool-facing schema to LLM-facing schema
   *
   ...
   * - Input blobs: { uri: string, contentType?: string } (uri is permanent like awp://...)
   * - Output blobs: { accept?: string, prefix?: string } (no url/uri - generated by runtime)
   */
  const toLlmFacingSchema = useCallback((tool: Tool): Record<string, unknown> => {
    const schema = JSON.parse(JSON.stringify(tool.inputSchema)) as {
      properties?: Record<string, Record<string, unknown>>;
      required?: string[];
    };

    if (!tool._awp?.blob || !schema.properties) {
      return schema;
    }
    ...
    for (const fieldName of Object.keys(blobInfo.output || {})) {
      ...
      schema.properties[fieldName] = {
        type: 'object',
        description: description || `Output blob: ${fieldName}`,
        properties: {
          accept: {
            type: 'string',
      ...
      // Remove from required array (output blobs are not required for LLM)
      if (schema.required) {
        schema.required = schema.required.filter((r) => r !== fieldName);
      }
    }
            ...
            <Grid item xs={12}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 2, mb: 1 }}>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={showLlmFacing}
                      onChange={(e) => setShowLlmFacing(e.target.checked)}
                      size="small"
                    />
                  }
                  label={
                    <Typography variant="body2" color="text.secondary">
                      {showLlmFacing ? 'LLM-facing Schema' : 'Tool-facing Schema'}
                    </Typography>
                  }
