import { EMPTY_COLLECTION_BYTES } from '../src/well-known.ts';
import { createHash } from 'crypto';

console.log('Empty collection bytes (hex):');
console.log(Buffer.from(EMPTY_COLLECTION_BYTES).toString('hex'));
...
const hash = createHash('sha256').update(EMPTY_COLLECTION_BYTES).digest('hex');
console.log('\nSHA-256 hash:');
