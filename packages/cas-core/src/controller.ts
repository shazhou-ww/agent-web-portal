...
/**
 * @deprecated Use individual functions with CasContext instead
 */
export class CasController {
  private ctx: CasContext;

  constructor(config: CasContext) {
    this.ctx = config;
  }

  async writeFile(data: Uint8Array, contentType: string): Promise<WriteResult> {
    return writeFile(this.ctx, data, contentType);
  }

  async putFileNode(data: Uint8Array, contentType?: string): Promise<string> {
    return putFileNode(this.ctx, data, contentType);
  }

  async makeCollection(entries: CollectionEntry[]): Promise<string> {
    return makeCollection(this.ctx, entries);
  }

  async getTree(rootKey: string, limit = 1000): Promise<TreeResponse> {
    return getTree(this.ctx, rootKey, limit);
  }

  async getChunk(key: string): Promise<Uint8Array | null> {
    return getChunk(this.ctx, key);
  }

  async getNode(key: string): Promise<CasNode | null> {
    return getNode(this.ctx, key);
  }

  async readFile(key: string): Promise<Uint8Array | null> {
    return readFile(this.ctx, key);
  }

  openFileStream(key: string): ReadableStream<Uint8Array> {
    return openFileStream(this.ctx, key);
  }

  async has(key: string): Promise<boolean> {
    return has(this.ctx, key);
  }

  getNodeLimit(): number {
    return getNodeLimit(this.ctx);
  }
}

/**
 * @deprecated Use CasContext instead
 */
export type CasControllerConfig = CasContext;
