...
When an image generation tool (like flux_pro, txt2img, etc.) returns a result, it includes a CAS blob reference. You MUST display it using Markdown.

Example tool result:
\`\`\`json
{
  "metadata": { "id": "...", "seed": 123 },
  "image": { "cas-node": "sha256:abc123def456...", "path": "." }
}
\`\`\`
...
![Generated image](cas://sha256:abc123def456...)
\`\`\`

**RULES:**
1. Extract the EXACT \`cas-node\` value from the \`image\` field (e.g., \`sha256:abc123def456...\`)
2. Use Markdown image syntax: \`![description](cas://CAS_NODE_HERE)\`
3. NEVER leave the src empty - always include the cas:// URI from the result
4. The URI starts with \`cas://\` followed by the CAS node key

Do NOT use the entire object - only use the \`cas-node\` string value.

The system will automatically resolve the CAS URI and display the image inline in the chat.
